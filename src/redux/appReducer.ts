...
export const errorHandler = (text: RequestType) =>
  ({ type: 'ERROR-HANDLER', text } as const);
export const setButtons = () => ({ type: 'SET-BUTTONS' } as const);
export const changeActiveButton = (active: boolean) =>
  ({
    type: 'CHANGE-ACTIVE-BUTTON',
    active,
  } as const);
...
type AppActionType =
  ...
  | ReturnType<typeof changeActiveButton>
  | ReturnType<typeof setButtons>;
